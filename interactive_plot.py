...
class BikeDash(param.Parameterized):
    ...
    """
    city = param.Selector(default=CITY, objects=['nyc', 'chic', 'washDC', 'minn', 'boston', 'london', 'helsinki', 'madrid', 'edinburgh'])
    if MONTH == None:
...
def service_area_plot(show_service_area, service_radius, service_area_color, city, use_road):
    ...
    if show_service_area == 'True':
        ...
        # keep_cols = zone_percents_columns.copy()
        # keep_cols.extend(['service_area', 'pop_density', 
        #                   'service_area_size'])
        vdims = zone_percents_columns.copy()
